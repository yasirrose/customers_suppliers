  ...
  methods: {
    register: function register() {
      ...
      this.$refs.registerForm.validate().then(function (success) {
        if (success) {
          _auth_jwt_useJwt__WEBPACK_IMPORTED_MODULE_7__["default"].register({
            username: _this.username,
            email: _this.userEmail,
            password: _this.password
          }).then(function (response) {
            _auth_jwt_useJwt__WEBPACK_IMPORTED_MODULE_7__["default"].setToken(response.data.accessToken);
            _auth_jwt_useJwt__WEBPACK_IMPORTED_MODULE_7__["default"].setRefreshToken(response.data.refreshToken);
            localStorage.setItem('userData', JSON.stringify(response.data.userData));

            _this.$ability.update(response.data.userData.ability);
